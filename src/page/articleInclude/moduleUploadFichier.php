...
<?php 
    ...
    if (isset($_FILES['fichier']) && $_FILES['fichier']['error'] == 0) {
        ...
        if ($_FILES['fichier']['size'] <= 10000000000) {
            ...
            if (in_array($extensionImage,$extensionArray)) {
                ...
                $dossier = '../../src/img/article/tampon/'.time().basename($_FILES['fichier']['name']);
                // Envoyer au fichier
                ...
                <div class="recapUpload">
                    ...
                    <img src="<?=$dossier?>" alt="<?=$_FILES['fichier']['name']?>" class="nouvelUpload" width=300px>
                </div>
                <?php
                if (isset($_POST['tampon']) && $_POST['tampon'] == 'oui') {
                    $pdo = dbAccess();
                    $requete = $pdo->prepare('INSERT INTO tampon(lien,auteurId) VALUES (?,?)');
                    $requete->execute(array($dossier,$_SESSION['user']['id']));
                    $requete->closeCursor();
                }
            } else {
                header('location: ../../src/pages/redactionArticles?choix=uploaderPhoto&error=true&message=extension non autorisée.');
                exit();
            }
        }
    }
